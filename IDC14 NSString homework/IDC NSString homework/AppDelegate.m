//
//  AppDelegate.m
//  IDC NSString homework
//
//  Created by Петр Первухин on 06.06.17.
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    // Level Pupil
    NSString* firstPart = [text substringToIndex:text.length/2];
    NSString* secondPart = [text substringFromIndex:text.length/2];
    
    NSLog(@"Part1: %@ \n part2:%@",firstPart,secondPart);
    ...
    NSMutableArray* arrayNSString = [[NSMutableArray alloc] init];
    
    for (NSString* string in array) {
        if ([string containsString:@"NSString"]){
            [arrayNSString addObject:string];
        }
    }
    NSLog(@"  ");
    NSLog(@"%@",arrayNSString);
